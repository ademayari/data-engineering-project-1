...
SELECT flight_id, flightnumber, departure_time, arrival_time, duration, stops, DATE("2023-04-07"), NULL
FROM airfares.flight
...
FROM DimFlight DF
INNER JOIN airfares.flight f ON DF.flight_id = f.flight_id
...
UPDATE DimFlight DF
LEFT JOIN airfares.flight f ON DF.flight_id = f.flight_id
SET DF.end_date = DATE_SUB(NOW(), INTERVAL 1 DAY)
